...
local emerald     = { r = 0.2, g = 0.6, b = 1.0 };
local errata      = { r = 1.0, g = 0.2, b = 0.2 };
local brown = {r= 0.5254901960784314,g = 0.5254901960784314,b = 0.6313725490196078,};
...
local function GetOverlayData( continent, zone, mapFileName )
	...
	local overlayData = continent 
					and zone 
					and mapFileName 
					and OMCache[continent] 
					and OMCache[continent].zones[zone] 
					and OMCache[continent].zones[zone].maps[mapFileName]
					or nil;	
...
if not OmegaMapConfig.showCompactMode then return end
		...
		local enabled       = DefaultOptions.enabled;
		local showKnownData = DefaultOptions.showKnownData;
		local labelPanels   = DefaultOptions.labelPanels and overlayName == "OmegaMapOverlay";
		local panelCache    = Panels[overlayName] or {};
		local labelCache    = Labels[overlayName] or {};
		local panelCount    = 0;
		
		-- we use this cache to keep track of what overlays we're manipulating
		
		Panels[overlayName] = panelCache;
		Labels[overlayName] = labelCache;
...
local function OmegaMapUpdateOverlays()
	UpdateOverlays(
		OmegaMapDetailFrame,
		"OmegaMapOverlay", 
		1, 
...
------------------------------------------------------------------------------------------------
		...
		BuildContinentInfo( Continents, -1, "Cosmic Map" );
		BuildContinentInfo( Continents, 0, "Azeroth" );
		
		for id,continent in pairs( continents ) do
			BuildContinentInfo( Continents, id, continent );
		end
		-- start building the map cache by plugging in what we know about the map
		-- as of the current version of MFWM
		
		OMCache = TableCopy( MapData );
------------------------------------------------------------------------------------------------

if OmegaMapFrame_Update then
	hooksecurefunc(	"OmegaMapFrame_Update", OmegaMapUpdateOverlays );
end
