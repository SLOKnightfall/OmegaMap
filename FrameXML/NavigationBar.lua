...
local NAVBAR_WIDTHBUFFER = 20;


function OM_NavBar_Initialize(self, template, homeData, homeButton, overflowButton)
	...
	if not self.dropDown then
		...
		self.dropDown = CreateFrame("Frame", dropDownName, self, "BW_UIDropDownMenuTemplate");
		BW_UIDropDownMenu_Initialize(self.dropDown, OM_NavBar_DropDown_Initialize, "MENU");
	end
	...
	if not homeButton:GetScript("OnEnter") then
		homeButton:SetScript("OnEnter",	OM_NavBar_ButtonOnEnter);
	end
	
	if not homeButton:GetScript("OnLeave") then
		homeButton:SetScript("OnLeave",	OM_NavBar_ButtonOnLeave);
	end
	...
	overflowButton.listFunc = OM_NavBar_ListOverFlowButtons;
	...
	homeButton:SetScript("OnClick", OM_NavBar_ButtonOnClick);
	overflowButton:SetScript("OnMouseDown", OM_NavBar_ToggleMenu);
...
function OM_NavBar_AddButton(self, buttonData)
	...
	if not navButton then
		...
		navButton:SetScript("OnClick", OM_NavBar_ButtonOnClick);
		if ( self.oldStyle ) then
		...
		if not navButton:GetScript("OnEnter") then
			navButton:SetScript("OnEnter",	OM_NavBar_ButtonOnEnter);
		end
		
		if not navButton:GetScript("OnLeave") then
			navButton:SetScript("OnLeave", OM_NavBar_ButtonOnLeave);
		end
...
function OM_NavBar_ClearTrailingButtons(list, freeList, button)
	...
	OM_NavBar_CheckLength(button:GetParent());
end

function OM_NavBar_OpenTo(self, id)
	local button;
	local found = false;
	...
	if (found) then
		OM_NavBar_ClearTrailingButtons(self.navList, self.freeButtons, button)
	end
end

function OM_NavBar_ButtonOnClick(self, button)
	local parent = self:GetParent()
	if button == "LeftButton" then
		OM_NavBar_ClearTrailingButtons(parent.navList, parent.freeButtons, self);
...
function OM_NavBar_ButtonOnEnter(self)
...
function OM_NavBar_ButtonOnLeave(self)
	if self.text:IsTruncated() then
		GameTooltip:Hide();
	end
end
...
function OM_NavBar_CheckLength(self)
	local width = 0;
...
function OM_NavBar_ListOverFlowButtons(self)
	local navBar = self:GetParent();
	local list = { };
	for i, button in ipairs(navBar.navList) do
		if not button:IsShown() then
			local entry = { text = button:GetText(), id = i, func = OM_NavBar_OverflowItemOnClick };
			tinsert(list, entry);
		else
			break;
		end
...
function OM_NavBar_ToggleMenu(self, button)
	self:GetParent().dropDown.buttonOwner = self;
	BW_ToggleDropDownMenu(nil, nil, self:GetParent().dropDown, self, 20, 3);
end


function OM_NavBar_DropDown_Initialize(self, level)
	...
	info.func = OM_NavBar_DropDown_Click;
...
function OM_NavBar_DropDown_Click(self, index, func)
	local navButton = self.owner;
	local navBar = navButton:GetParent();
	
	if func ~= OM_NavBar_OverflowItemOnClick then
		OM_NavBar_ClearTrailingButtons(navBar.navList, navBar.freeButtons, navButton);
	end
...
function OM_NavBar_OverflowItemOnClick(junk, index, navBar)
