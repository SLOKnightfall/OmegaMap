...
function OmegaMapBountyBoardMixin:ShowBountyTooltip(bountyIndex)
	...
	if title then
		...
		if bountyData.turninRequirementText then
			OmegaMapTooltip:AddLine(bountyData.turninRequirementText, RED_FONT_COLOR.r, RED_FONT_COLOR.g, RED_FONT_COLOR.b, true);
		end
...
function	OmegaMapBountyBoardMixin:InvalidateMapCache()
	self.cachedMapInfo = nil;
end

function OmegaMapBountyBoardMixin:CalculateNumActiveWorldQuestsForSelectedBountyByMap(areaMapID, parentAreaMapID)
	local numQuests = 0;
...
function OmegaMapBountyBoardMixin:CacheMapsForSelectionBounty()
...
function OmegaMapBountyBoardMixin:FindCachedZoneMapIndexFromZoneAreaID(zoneMapAreaID)
...
function OmegaMapBountyBoardMixin:FindCachedZoneIndexFromContinentAreaID(continentMapAreaID)
	local continents = { GetMapContinents() };
	for continentElementIndex = 1, #continents, 2 do
		local continentIndex = (continentElementIndex + 1) / 2;
...
function OmegaMapBountyBoardMixin:FindCachedZoneIndexFromCurrentMapAreaID()
...
function OmegaMapBountyBoardMixin:FindNextBestMapIndex(currentIndex)
	assert(self.cachedMapInfo);
