...
-- Code for HereBeDragons  Integration 
-- Code is a modified version of HereBeDragons-Pins.lua from HereBeDragons-1.0 (v1.08-release)
-- HereBeDragons is written by Nevcairiel @ http://www.curse.com/addons/wow/herebedragons

--	///////////////////////////////////////////////////////////////////////////////////////////


--if IsAddOnLoaded("HereBeDragons-1.0") then
...
-- and omegadmap pins  -Added
pins.omegamapPins         = pins.worldmapPins or {}
pins.omegamapPinRegistry  = pins.worldmapPinRegistry or {}
...
local omegamapWidth, omegamapHeight = OmegaMapButton:GetWidth(), OmegaMapButton:GetHeight()
...
local function PositionOmegaMapIcon(icon, data, currentMapID, currentMapFloor)
    local x, y = HBD:GetZoneCoordinatesFromWorld(data.x, data.y, currentMapID, currentMapFloor)
    if x and y then
        icon:ClearAllPoints()
        icon:SetPoint("CENTER", OmegaMapButton, "TOPLEFT", x * omegamapWidth, -y * omegamapHeight)
        icon:Show()
    else
        icon:Hide()
    end
...
local function UpdateOmegaMap()
    ...
    -- not viewing a valid map
    if not mapID or mapID == -1 then
        for icon in pairs(omegamapPins) do
            icon:Hide()
        end
        return
    end
    ...
    omegamapWidth  = OmegaMapButton:GetWidth()
    omegamapHeight = OmegaMapButton:GetHeight()

    for icon, data in pairs(omegamapPins) do
        if instanceID == data.instanceID and (not data.floor or (data.floor == mapFloor and (data.floor == 0 or data.mapID == mapID))) then
            PositionOmegaMapIcon(icon, data, mapID, mapFloor)
        else
            icon:Hide()
        end
    end
...
function pins:AddWorldMapIconWorld(ref, icon, instanceID, x, y)
    ...
    if not omegamapPinRegistry[ref] then
        omegamapPinRegistry[ref] = {}
    end

    worldmapPinRegistry[ref][icon] = true
    omegamapPinRegistry[ref][icon] = true
    ...
    if OmegaMapButton:IsVisible() then
        local currentMapID, currentMapFloor = GetCurrentMapAreaID(), GetCurrentMapDungeonLevel()
        if currentMapID and HBD.mapData[currentMapID] and HBD.mapData[currentMapID].instance == instanceID then
            PositionOmegaMapIcon(icon, t, currentMapID, currentMapFloor)
        else
            icon:Hide()
        end
    end
