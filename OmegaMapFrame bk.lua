...
function OmegaMapFrame_OnEvent(self, event, ...)
	...
	elseif ( event == "VARIABLES_LOADED" ) then
		...
		OmegaMapBlobFrame:SetScale(OmegaMapConfig.scale);
		OmegaMapScenarioPOIFrame:SetScale(OmegaMapConfig.scale);	--If we ever need to add objectives on the map itself we should adjust this value
...
function OmegaMapEffectPOI_OnEnter(self)
	if(OmegaMapEffectPOITooltips[self.name] ~= nil) then
		OmegaMapTooltip:SetOwner(self, "ANCHOR_RIGHT");
...
OmegaMapMasterFrame:SetFrameStrata("HIGH")
	...
	OmegaMapPOIFrame:SetFrameLevel(WORLDMAP_POI_FRAMELEVEL);
		OmegaMapNoteFrame:SetFrameLevel(WORLDMAP_POI_FRAMELEVEL);
