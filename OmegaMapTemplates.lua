OmegaMapFloorNavigationFrameMixin = { }

function OmegaMapFloorNavigationFrameMixin:OnLoad()
	UIDropDownMenu_SetWidth(self, 130);
end

function OmegaMapFloorNavigationFrameMixin:Refresh()
...
function OmegaMapFloorNavigationFrameMixin:InitializeDropDown()
	local mapID = self:GetParent():GetMapID();
	local mapGroupID = C_Map.GetMapGroupID(mapID);
...
OmegaMapTrackingOptionsButtonMixin = { };

function OmegaMapTrackingOptionsButtonMixin:OnLoad()
...
function OmegaMapTrackingOptionsButtonMixin:OnMouseDown(button)
...
function OmegaMapTrackingOptionsButtonMixin:OnMouseUp()
	self.Icon:SetPoint("TOPLEFT", self, "TOPLEFT", 6, -6);
	self.IconOverlay:Hide();
end

function OmegaMapTrackingOptionsButtonMixin:Refresh()
	-- nothing to do here
end

function OmegaMapTrackingOptionsButtonMixin:OnSelection(value, checked)
	if (checked) then
		PlaySound(SOUNDKIT.IG_MAINMENU_OPTION_CHECKBOX_ON);
	else
...
function OmegaMapTrackingOptionsButtonMixin:InitializeDropDown()
...
function OmegaMapNavBarMixin:GoToMap(mapID)
...
function OmegaMapNavBarMixin:Refresh()
	...
	while mapInfo and mapInfo.parentMapID > 0 do
		local buttonData = {
			name = mapInfo.name,
			id = mapInfo.mapID,
			OnClick = OmegaMapNavBarButtonMixin.OnClick,
		};
		if ( IsMapValidForNavBarDropDown(mapInfo) ) then
			buttonData.listFunc = OmegaMapNavBarButtonMixin.GetDropDownList;
		end
...
function OmegaMapNavBarButtonMixin:OnClick()
	self:GetParent():GoToMap(self.data.id)
end

OmegaMapSidePanelToggleMixin = { };

function OmegaMapSidePanelToggleMixin:OnClick()
	self:GetParent():HandleUserActionToggleSidePanel();
	self:Refresh();
end

function OmegaMapSidePanelToggleMixin:Refresh()
...
OmegaMapZoneTimerMixin = {};

function OmegaMapZoneTimerMixin:OnUpdate(elapsed)
...
function OmegaMapZoneTimerMixin:Refresh()
	-- nothing to do here
end


OmegaMapThreatFrameMixin = {};
function OmegaMapThreatFrameMixin:OnLoad()
	self:RegisterEvent("UI_MODEL_SCENE_INFO_UPDATED");
	self.dirtyModels = true;
end
function OmegaMapThreatFrameMixin:OnShow()
	self:Refresh();
	self:RegisterEvent("QUEST_ACCEPTED");
	self:RegisterEvent("QUEST_REMOVED");
end
function OmegaMapThreatFrameMixin:OnHide()
	self:UnregisterEvent("QUEST_ACCEPTED");
	self:UnregisterEvent("QUEST_REMOVED");
end
function OmegaMapThreatFrameMixin:OnEvent(event)
...
function OmegaMapThreatFrameMixin:Refresh()
	local show = false;
...
function OmegaMapThreatFrameMixin:RefreshModels()
...
function OmegaMapThreatFrameMixin:SetNextMapForThreat()
	local threatMaps = C_QuestLog.GetActiveThreatMaps();
...
OmegaMapThreatEyeMixin = { };
function OmegaMapThreatEyeMixin:OnShow()
...
function OmegaMapThreatEyeMixin:OnEnter()
...
function OmegaMapThreatEyeMixin:OnMouseDown()
