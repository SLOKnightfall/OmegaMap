...
function OM_CampaignOverviewMixin:OnLoad()
	self.ScrollFrame.ScrollBar:ClearAllPoints();
	self.ScrollFrame.ScrollBar:SetAllPoints(OM_QuestScrollFrame.ScrollBar);
...
do
	...
	function OM_CampaignOverviewMixin:OnShow()
...
function OM_CampaignOverviewMixin:OnEvent(event, ...)
...
function OM_CampaignOverviewMixin:SetCampaign(campaignID)
...
function OM_CampaignOverviewMixin:SetupEntry(index, entry)
...
function OM_CampaignOverviewMixin:SetupEntryHeader(index, entry, line)
	entry.text = "\n" .. entry.text;
...
function OM_CampaignOverviewMixin:SetupEntryStandard(index, entry, line)
...
function OM_CampaignOverviewMixin:SetupEntryData(index, entry, line)
	line:SetText(entry.text);
	line:Show();
end

function OM_CampaignOverviewMixin:UpdateCampaignLoreText(campaignID, textEntries)
	self.linePool:ReleaseAll();
	self.texturePool:ReleaseAll();
