...
-- Code to add world qyest to the OmegaMap Quest loggaMap
-- This is modified code taken from .lua from AngryWorldQuests 
-- CTMapMod is written and maintained by the 
...
--local QF = AngryWorldQuests:NewModule('QuestFrame')
--local ADDON, Addon = ...
local QF = Addon:NewModule('OMQuestFrame')
local Config
...
local function DisplayMyTaskPOI(self)
	if GetCurrentMapAreaID() == MAPID_BROKENISLES and Config.showHoveredPOI and not Config.showContinentPOI then
		...
		local isCriteria = OmegaMapFrame.UIElementsFrame.BountyBoard:IsWorldQuestCriteriaForSelectedBounty(self.questID)
		...
		OmegaMap_SetupWorldQuestButton(myTaskPOI, worldQuestType, rarity, isElite, tradeskillLineIndex, self.inProgress, selected, isCriteria, isSpellTarget)
		OmegaMapPOIFrame_AnchorPOI(myTaskPOI, self.infoX, self.infoY, WORLD_MAP_POI_FRAME_LEVEL_OFFSETS.WORLD_QUEST)
...
local function TitleButton_OnEnter(self)
	...
	for i = 1, NUM_OMEGAMAP_TASK_POIS do
		local mapButton = _G["OmegaMapFrameTaskPOI"..i]
...
local function FilterButton_ShowMenu(self)
	if not filterMenu then
		filterMenu = CreateFrame("Button", "DropDownMenuAWQ", OmegaMapQuestFrame, My_UIDropDownMenuTemplate)
	end
...
local function GetTitleButton(questID)
	if ( not titleButtons[questID] ) then
		local title = CreateFrame("BUTTON", nil, OmegaMapQuestFrame.QuestsFrame.Contents, "QuestLogTitleTemplate")
...
local function GetObjectiveButton(index)
	if ( not objectiveButtons[index] ) then
		local frame = CreateFrame("FRAME", "AWQOF"..index, OmegaMapQuestFrame.QuestsFrame.Contents, "QuestLogObjectiveTemplate")
...
local function GetFilterButton(index)
	if ( not filterButtons[index] ) then
		local button = CreateFrame("Button", nil, OmegaMapQuestFrame.QuestsFrame.Contents)
		button.index = index
...
local function QuestFrame_Update()
	if not OmegaMapFrame:IsShown() then return end
	...
	if ( storyID ) then
		storyButton = OM_QuestScrollFrame.Contents.StoryHeader
	...
	for i = 1, numEntries do
		...
		elseif ( not isTask and not isHidden and (not isBounty or IsQuestComplete(questID))) then
			if ( not headerShown ) then
				...
				prevButton = OM_QuestLogQuests_GetHeaderButton(headerIndex)
				if not firstButton then firstButton = prevButton end
			end
			if (not headerCollapsed) then
				titleIndex = titleIndex + 1
				prevButton = OM_QuestLogQuests_GetTitleButton(titleIndex)
			end
		end
	end

	OM_QuestScrollFrame.Background:SetAtlas("QuestLogBackground", true) -- Always show quest background
			...
			if (questsList and #questsList > 0) then
				for i, questInfo in ipairs(questsList) do
					...
					if (HaveQuestData (questID)) then
						...
						local isWorldQuest = OmegaMapQuestFrame_IsQuestWorldQuest(questID)
						local passFilters = OmegaMap_DoesWorldQuestInfoPassFilters(questInfo)
						local isSuppressed = OmegaMap_IsWorldQuestSuppressed(questID)
			...
			if (questsList and #questsList > 0) then
				for i, info in ipairs(questsList) do
					if ( HaveQuestData(info.questId) ) then
						local isWorldQuest = OmegaMapQuestFrame_IsQuestWorldQuest(info.questId);
						if isWorldQuest then
							local taskPOI = OmegaMap_TryCreatingWorldQuestPOI(info, "AWQ"..taskIconIndex)
print(taskIconIndex)
							if ( taskPOI ) then
								OmegaMapPOIFrame_AnchorPOI(taskPOI, info.x, info.y, WORLD_MAP_POI_FRAME_LEVEL_OFFSETS.WORLD_QUEST)
		...
		for i = taskIconIndex, AWQ_POI_COUNT do
			_G["OmegaMapFrameTaskPOIAWQ"..i]:Hide()
			_G["OmegaMapFrameTaskPOIAWQ"..i].wasShown = false
		end
		...
		for i = 1, AWQ_POI_COUNT do
			_G["OmegadMapFrameTaskPOIAWQ"..i]:Hide()
			_G["OmegaMapFrameTaskPOIAWQ"..i].wasShown = false
		end
	end
	...
	if Config.hideUntrackedPOI then
		...
		for i = 1, AWQ_POI_COUNT do
			local taskPOI = _G["OmegaMapFrameTaskPOIAWQ"..i]
			if taskPOI.worldQuest and taskPOI:IsShown() and not (IsWorldQuestHardWatched(taskPOI.questID) or GetSuperTrackedQuestID() == taskPOI.questID) then
				taskPOI:Hide()
			end
		end
...
function QF:UNIT_INVENTORY_CHANGED(unit)
	if (Config.filterLoot == FILTER_LOOT_UPGRADES or (Config.filterLoot == 0 and Config.lootFilterUpgrades)) and unit == "player" then
		QuestFrame_Update() 
		if Config.hideFilteredPOI and OmegaMapFrame:IsVisible() then
...
function AddQuestTimeToTooltip(questID)
	...
	if timeLeftMinutes and timeLeftMinutes > 0 and timeLeftMinutes >= 24 * 60 + WORLD_QUESTS_TIME_CRITICAL_MINUTES then
		...
		--_G["OmegaMapTooltipTextLeft"..OmegaMapTooltip:NumLines()]:SetText(BONUS_OBJECTIVE_TIME_LEFT:format(timeString), color.r, color.g, color.b)
	end
end
...
function QF:Startup()
	...
	Config:RegisterCallback({'showAtTop', 'showEverywhere'}, function() OmegaMapQuestFrame_UpdateAll(); QuestFrame_Update() end)
	Config:RegisterCallback({'hideUntrackedPOI', 'hideFilteredPOI', 'showContinentPOI'}, function() OmegaMap_UpdateQuestBonusObjectives(); MapFrame_Update() end)
	...
	Config:RegisterCallback({'selectedFilters', 'disabledFilters', 'filterEmissary', 'filterLoot', 'filterFaction', 'filterZone', 'filterTime', 'lootFilterUpgrades', 'lootUpgradesLevel', 'timeFilterDuration'}, function() 
		...
		if Config.hideFilteredPOI and OmegaMapFrame:IsShown() then
			OmegaMap_UpdateQuestBonusObjectives()
			MapFrame_Update()
		end
	end)
	...
	hooksecurefunc("OM_QuestLogQuests_Update", QuestFrame_Update)
	hooksecurefunc("OmegaMapTrackingOptionsDropDown_OnClick", QuestFrame_Update)
	hooksecurefunc("OmegaMap_UpdateQuestBonusObjectives", UpdateQuestBonusObjectives)
	hooksecurefunc("OmegaMap_AddQuestTimeToTooltip", AddQuestTimeToTooltip)

	myTaskPOI = OmegaMap_GetOrCreateTaskPOI("AWQ0")
	myTaskPOI:Hide()
end
