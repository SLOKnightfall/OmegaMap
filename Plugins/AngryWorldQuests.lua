...
--local ADDON, Addon = ...
local Mod = Addon:NewModule('OMQuestFrame')
local Config
...
local function TitleButton_OnClick(self, button)
	...
	if ( not ChatEdit_TryInsertQuestLinkForQuestID(self.questID) ) then
		...
		if ( button == "RightButton" ) then
			if ( self.mapID ) then
				OM_QuestMapFrame:GetParent():SetMapID(self.mapID)
			end
...
local function FilterButton_ShowMenu(self)
	if not filterMenu then
		filterMenu = CreateFrame("Button", "DropDownMenuAWQ", OM_QuestMapFrame, My_UIDropDownMenuTemplate)
	end
...
local function GetFilterButton(key)
	...
	if ( not filterButtons[index] ) then
		local button = CreateFrame("Button", nil, OM_QuestMapFrame.QuestsFrame.Contents)
		button.filter = key
...
local function TaskPOI_IsFiltered(info)
	...
	if hasFilters then
		...
		if selectedFilters.ZONE then
			local currentMapID = OM_QuestMapFrame:GetParent():GetMapID()
...
local function QuestFrame_Update()
	...
	for header in OM_QuestScrollFrame.headerFramePool:EnumerateActive() do
	...
	if not headerButton then
		headerButton = CreateFrame("BUTTON", nil, OM_QuestMapFrame.QuestsFrame.Contents, "QuestLogHeaderTemplate")
	...
	headerButton.layoutIndex = OM_QuestMapFrame:GetManagedLayoutIndex("AWQ")
	headerButton:Show()
	...
	if questsCollapsed then
		...
		for i, button in ipairs(usedButtons) do
			button.layoutIndex = OM_QuestMapFrame:GetManagedLayoutIndex("AWQ")
			button:Show()
			prevButton = button
	...
	if not spacerFrame then
		spacerFrame = CreateFrame("FRAME", nil, OM_QuestMapFrame.QuestsFrame.Contents)
		spacerFrame:SetHeight(6)
	end
	if #usedButtons > 0 then
		spacerFrame:Show()
		spacerFrame.layoutIndex = OM_QuestMapFrame:GetManagedLayoutIndex("AWQ")
	else
		spacerFrame:Hide()
	end
...
function Mod:Blizzard_WorldMap()
	for dp,_ in pairs(OmegaMapFrame.dataProviders) do
...
local function OverrideLayoutManager()
	if Config.showAtTop then
		OM_QuestMapFrame.layoutIndexManager.startingLayoutIndexes["Other"] = QUEST_LOG_STORY_LAYOUT_INDEX + 500 + 1
		OM_QuestMapFrame.layoutIndexManager:AddManagedLayoutIndex("AWQ", QUEST_LOG_STORY_LAYOUT_INDEX + 1)
	else
		OM_QuestMapFrame.layoutIndexManager.startingLayoutIndexes["Other"] = QUEST_LOG_STORY_LAYOUT_INDEX + 1
		OM_QuestMapFrame.layoutIndexManager:AddManagedLayoutIndex("AWQ", QUEST_LOG_STORY_LAYOUT_INDEX + 500 + 1)
	end
end
...
function Mod:Startup()
	...
	titleFramePool = CreateFramePool("BUTTON", OM_QuestMapFrame.QuestsFrame.Contents, "QuestLogTitleTemplate")
